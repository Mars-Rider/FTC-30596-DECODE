...
@Autonomous(name = "9 Ball", group = "Autons")
public class NineBall extends OpMode {
    ...
    private Pose scorePose = new Pose(40, 85, Math.toRadians(-45)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private Pose parkPose = new Pose(30, 60, Math.toRadians(-180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private Pose set1 = new Pose(60, 50, Math.toRadians(-180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private Pose grab1 = new Pose(10, 50, Math.toRadians(-180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private Pose set2 = new Pose(60, 75, Math.toRadians(-180)); // Highest (First Set) of Artifacts from the Spike Mark.
    ...
    public void buildPaths() {
        ...
        scoreRow1 = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(set1.getHeading(), scorePose.getHeading() - (Globals.alliance == 1 ? Math.toRadians(15) : Math.toRadians(-15)))
                .build();
        ...
        scoreRow2 = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(set2.getHeading(), scorePose.getHeading() - (Globals.alliance == 1 ? Math.toRadians(10) : Math.toRadians(-10)))
                .build();
    ...
    public void init() {
        ...
        scorePose = new Pose(robot.changeAlliance(scorePose.getX()), 89, Globals.alliance == 1 ? Math.toRadians(-45) : Math.toRadians(-135)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
        parkPose = new Pose(robot.changeAlliance(parkPose.getX()), 60, Globals.alliance == 1 ? Math.toRadians(-180) : Math.toRadians(0));
